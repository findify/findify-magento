...
{	
	...
	$extradata = array(
	    ...
	    foreach ($website->getGroups() as $group) {
		    ...
		    Mage::log('Findify: starting feed generation for store '.$websiteCode.'-'.$storeCode.' (ID: '.$storeId.')');
			...
			if(empty($filename)){
				$filename = 'jsonl_feed-'.$websiteCode.'-'.$storeCode;
			}
			...
			if ($selectedattributes) {
			    ...
			    if (is_array($selectedattributes)) {
			        foreach($selectedattributes as $selectedattributesRow) {
                        ...
                        Mage::getResourceModel('catalog/product_collection')->setStore($storeId); // workaround for Magento getResourceModel bug when using flat product
			$products = Mage::getResourceModel('catalog/product_collection')
			    ->addAttributeToFilter('status',array('eq' => Mage_Catalog_Model_Product_Status::STATUS_ENABLED))
			    ->addAttributeToSelect($attributesUsed)
			    ->addAttributeToFilter('visibility',array('in' => array(2,3,4))); // skip product if it is not visible
